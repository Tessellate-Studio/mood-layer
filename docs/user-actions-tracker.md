...
| Pulse scoring dependency | ✅ | Superseded 2026-07-17 — rubric-sdk was merged into `@tessellate-studio/forge` (PR #21, `07e84af`), which is the dep in `package.json` today. Nothing left to do; scoring stays numeric via forge's `evaluateFromContext`. *(History: rubric-sdk installed 2026-07-12; the older "agent sandbox can't install it" note was wrong — it installed cleanly.)* |
| EAS dev build (gates **four** on-device checks) | 🟡 | Expo Go can't fire local scheduled notifications (expo-notifications removed SDK 53+ — regression-log #4), so this one build is the gate for: (a) "Name it" reminders firing + deep-linking into check-in; (b) Circle scheduled-share reminders firing (PR #13, `c547258` — circle service fns no-op under Expo Go); (c) the Phase-2 background relay wake + "a week arrived" notification (Circle relay §6); (d) an on-device look at the new app icons (PR #14, `e862632`). `eas.json` is scaffolded (dev/preview/production) and EAS login is done. Left: `eas build --profile development --platform android`, install the dev client, walk those four. Everything *else* is testable in Expo Go |
